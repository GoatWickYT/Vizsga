 ...
 */
export const getAllStatusses = async (): Promise<Status[]> => {
    return await queryRows<Status>('SELECT * FROM statuses');
};
 ...
 */
export const getSingleStatus = async (id: number): Promise<Status | null> => {
    const rows = await queryRows<Status>('SELECT * FROM statuses WHERE id = ?', [id]);
    return rows[0] || null;
};
 ...
 */
export const createStatus = async (Status: Status): Promise<number> => {
    const result = await queryExec('INSERT INTO statuses (name, age, spot_id) VALUES (?, ?, ?)', [
        Status.name,
        Status.age,
        Status.spotId,
    ]);
 ...
 */
export const updateStatus = async (id: number, Status: Partial<Status>): Promise<boolean> => {
    const result = await queryExec('UPDATE statuses SET name = ?, age = ? WHERE id = ?', [
        Status.name,
        Status.age,
        id,
    ]);
...
export const deleteStatus = async (id: number): Promise<boolean> => {
    const result = await queryExec('DELETE FROM statuses WHERE id = ?', [id]);
    return result.affectedRows > 0;
};
