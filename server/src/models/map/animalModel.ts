...
export async function getAllAnimals(): Promise<Animal[]> {
    return queryRows<Animal>('SELECT * FROM animals');
}
 ...
 */
export async function getSingleAnimal(id: number): Promise<Animal | null> {
 ...
 */
export async function createAnimal(Animal: Animal): Promise<number> {
 ...
 */
export async function updateAnimal(id: number, Animal: Partial<Animal>): Promise<boolean> {
    const result = await queryExec(
